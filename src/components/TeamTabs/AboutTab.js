...
const AboutTab = ({ member }) => {
  return (
    <View style={styles.container}>
      <Typography style={styles.sectionHeader}>Personal Details</Typography>

      <View style={styles.detailsGroup}>
        <InfoItem icon="user" label="Full Name" value={member?.name || 'Sarah Lee'} />
        <InfoItem icon="calendar" label="Date of Birth" value="January 1, 1987" />
        <InfoItem icon="target" label="Gender" value="Female" />
        <InfoItem icon="map-pin" label="Address" value={"225 Cherry Street #24,\nNew York, NY"} />
        <InfoItem icon="briefcase" label="Joining Date" value="Jan 12, 2024" />
      </View>
      ...
      <View style={styles.detailsGroup}>
        <InfoItem icon="mail" label="Email" value="sarah@studio.com" />
